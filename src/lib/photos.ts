...
function tagValue<T = string>(tag: { value?: T; description?: string } | undefined): T | undefined {
	return tag?.value ?? undefined;
}

function tagDescription(tag: { description?: string } | undefined): string | undefined {
...
export async function getPhotos(): Promise<Photo[]> {
	...
	photos.sort((a, b) => {
		if (a.exif.date && b.exif.date) return b.exif.date.getTime() - a.exif.date.getTime();
